const {
  BinarySearchTree,
} = require("../checkpoint/BinarySearchTreeSpotHousePrices");
...
describe("Ejercicio 10 - ingresoEmpleado", () => {
  test("Debería retornar true si los árboles son iguales", () => {
    ...
    expect(anotherHousePrices.spotHousePrices(6, [])).toEqual([
      13, 8, 7, 15, 14, 16, 20,
    ]);
    expect(anotherHousePrices.spotHousePrices(14, [])).toEqual([15, 16, 20]);
    expect(anotherHousePrices.spotHousePrices(4, [])).toEqual([
      13, 8, 7, 5, 15, 14, 16, 20,
    ]);
  });
});
